var MMUnlockLayer = cc.LayerColor.extend({
	...
	loadBackground : function(){
		var node = new ccui.ImageView(res.WoodBgNotice_png);
	...
	// 加载[图片说明]
	loadInfo : function(){
		var node = new ccui.ImageView(res.UnlockFloor_png);
	...
	loadConfirmButton : function(offsetX, posY){
		...
		this.background.addChild(node);
		node.loadTextures(res.BtnBlueM_png, res.BtnBlueMPressed_png, "");
		...
		// 【确定】文字图片
		var infoNode = new ccui.ImageView(res.BtnBlueMOk_png);
	...
	loadCancelButton : function(offsetX, posY){
		var node = new ccui.Button();
		this.background.addChild(node);
		node.loadTextures(res.BtnGreenM_png, res.BtnGreenMPressed_png, "");
		node.setPosition(this.background.width / 2 + node.width / 2 + offsetX, posY);
		node.addTouchEventListener(function(sender, type){
			switch (type) {
				case ccui.Widget.TOUCH_ENDED:
		...
		var infoNode = new ccui.ImageView(res.BtnGreenMCancel_png);
		node.addChild(infoNode);
		infoNode.setPosition(node.width / 2, node.height / 2);
	}
});